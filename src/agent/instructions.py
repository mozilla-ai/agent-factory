...
{{
    "answer": "The answer to the user's question, generated by the agent. Use this field to return
        simple text answers to the user, if you want more information or no you're just chatting
        with them.",
    "code_ready": "Set this to `True`, if the agent has completed the user assigned task and
        provided the `agent_instructions`, `tools`, `imports`, `structured_outputs`,
        `agent_description`, `readme` and `dependencies`. Set this to `False`, othewise.",
    ...
    "readme": "The instructions for setting up the environment in Markdown format (e.g., a README
        file).",
...
8. `readme` should contain the title and the description of the generated agent, as well as a clear
   and concise setup instructions:
    ...
    - Run instructions for `agent.py` using `uv run` with specification of `requirements.txt` and
      ...
      where the user is expected to specify the required arguments
9. dependencies should list all the python libraries (including the ones required by the tools) as
   dependencies to be installed. It will be used to generate the `requirements.txt` file
    - the first line should be "any-agent[all,a2a]=={ANY_AGENT_VERSION}" dependency, since we are
      using any-agent to run the agent workflow.
    - only if the `agent_code` uses `uvx` to spin up any MCP server, include "uv" as a dependency in
      the `requirements.txt` file
    - do not provide specific versions for the dependencies except for `any-agent[all,a2a]` (see the
      above point)
"""  # noqa: E501
...
1. Before starting the agent code generation, you should read the task description carefully to
   understand what the user wants you to do. Always think of a few questions that you would like to
   ask the user to clarify the task, so you have more details. To ask the user, fill the `answer`
   field with your questions in Markdown, and set `final_answer` to `False`. This should be the
   response you will return to the user.
2. If you have enough information about the task, you can proceed with generating the plan you will
   ...
   plan, and set `final_answer` to `False`. This should be the response you will return to the user.
3. If the user confirms your plan, you can proceed with generating the agent code. If the user does
   ...
   `answer` field with your amended plan, and set `final_answer` to `False`. In the second case,
...
4. If you are ready to generate the agent code, you can fill the `answer` field with a confirmation
   saying "Your agent is ready!", and set `final_answer` to `True`. Also fill the
