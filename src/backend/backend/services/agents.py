...
class AgentService:
    ...
    async def create_agent(self, request: AgentCreateRequest) -> AgentSummary:
        ...
        extended_prompt = self._build_prompt(request.prompt)
        try:
            send_message_task.delay(extended_prompt, agent_id=str(record.id))
    ...
    def get_agent(self, agent_id: uuid.UUID) -> AgentConfig:
        ...
        Raises:
            AgentNotFoundError: If the agent with the specified ID does not exist.
        """
