...
from backend.templates import AGENT_CODE_TEMPLATE, CONTAINERFILE_TEMPLATE
...
def _save_agent(result: dict[str, str], agent_id: uuid.UUID) -> None:
    ...
    try:
        ...
        storage_client.put_object(
            Bucket=bucket_name,
            Key=f"{agent_id}/Containerfile",
            Body=io.BytesIO(CONTAINERFILE_TEMPLATE.encode()),
            ContentType="text/plain",
        )

        tools_dir = Path("backend/tools")
        for tool_file in tools_dir.iterdir():
            if tool_file.is_file() and tool_file.suffix == ".py":
