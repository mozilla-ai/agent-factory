...
def _update_agent_download(agent_id: uuid.UUID, available: bool):
    logger.info(f"Updating agent {agent_id} download_available to {available}")
    for repo in _get_agent_repository():
        repo.update(agent_id, download_available=available)


def _save_agent(result: dict[str, str], agent_id: uuid.UUID) -> None:
    bucket_name = settings.S3_BUCKET_NAME
    logger.info(f"Saving agent files to bucket {bucket_name} for agent {agent_id}...")
    try:
    ...
    try:
        ...
        storage_client.put_object(
            Bucket=bucket_name,
            Key=f"{agent_id}/README.md",
            Body=io.BytesIO(result["readme"].encode()),
        ...
        _update_agent_download(agent_id, available=True)
        ...
        try:
            ...
            if response:
                if isinstance(response.root, JSONRPCErrorResponse):
                    raise ValueError(f"Error from manufacturing agent: {response.error.message}")
                if isinstance(response.root, SendMessageSuccessResponse):
                    logger.info(f"Task completed successfully for agent {agent_id}.")
