...
class TestRegisterMcpServer:
    ...
    def test_register_mcp_server_name_only(self):
        ...
        with patch("agent_factory.utils.mcpd_utils.run_binary") as mock_run_binary:
            ...
            result = register_mcp_server(name="github", config_path=config_path)
    ...
    def test_register_mcp_server_with_version(self):
        ...
        with patch("agent_factory.utils.mcpd_utils.run_binary") as mock_run_binary:
            ...
            config_path = Path("/test/config.toml")
            result = register_mcp_server(name="github", config_path=config_path, version="v1.2.3")
    ...
    def test_register_mcp_server_with_tools(self):
        ...
        with patch("agent_factory.utils.mcpd_utils.run_binary") as mock_run_binary:
            ...
            result = register_mcp_server(name="github", config_path=config_path, tools=tools)
    ...
    def test_register_mcp_server_all_parameters(self):
        ...
        with patch("agent_factory.utils.mcpd_utils.run_binary") as mock_run_binary:
            ...
            config_path = Path("/test/config.toml")
            tools = ["tool1", "tool2"]
            result = register_mcp_server(name="slack", config_path=config_path, version="v2.0.0", tools=tools)
...
class TestExportMcpdConfigArtifacts:
    ...
    def test_export_artifacts_success(
        ...
        def truediv_side_effect(file_name):
            if file_name == ".mcpd.toml":
                return mock_mcpd_toml
            if file_name == ".env":
                return mock_env_contract
            if file_name == "secrets.prod.toml":
                return mock_secrets_context
            return MagicMock()

        mock_work_dir.__truediv__.side_effect = truediv_side_effect
        ...
        mock_init_config.assert_called_once_with(mock_mcpd_toml)

        assert mock_register_server.call_count == 2
        mock_register_server.assert_any_call("github", config_path=mock_mcpd_toml, tools=["list_repos"])
        mock_register_server.assert_any_call("jira", config_path=mock_mcpd_toml, tools=None)
    ...
    def test_export_with_output_dir(
        ...
        mock_init_config.assert_called_once_with(mock_mcpd_toml)
        mock_register_server.assert_called_once_with("github", config_path=mock_mcpd_toml, tools=None)
